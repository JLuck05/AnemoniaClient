...
* First of all the program set up an UDP Client, getting IP adress and port through configuration file. 
* Secondly builds up a chained list of sensors structs and performs the command passed as argument by the command line
* sending th right commands to the remote UDP Server. 
*
*/
#include <sys/types.h>
...
#define canaliTot 8 
#define MAXROWDIM 40
#define directory "/media/usb1/samples/seafloor/"
#define directoryTest "./"
#define zlogConfDir "./zlog.conf"
/*-------------------------------------------------------------------------*/
				/*Codici errori*/
/*
0 = Success
...
void powerOff(sensorList *sL, int s); // power off each channel, if sl is null, power off all channels
...
void salvaMisure(sensorList *sL);
...
void getvalues(duinoData **h, duinoData **t, int s, int numPack, sensorList * sL);
void cercaSensoreGetValues(int canale, char *misura,sensorList *sList);
void salvaAudio(duinoData **h, sensorList *sL);
void fill(duinoData **h, duinoData **t, int s, sensorList *sL);
...
int main(int argc, char *argv[]){
	...
	sensorLst=parse_ini_file("config.ini", &params);
	if(sensorLst == NULL){
	...
	if(s < 0){
		zlog_fatal(c,"error during socket creation");
		exit(1);
		}
	else{//parse arguments and send them to UDP Server

		for(arguments=1; arguments<argc;arguments++ ){
			if(strcmp(argv[arguments],"?")==0){
				buffer="?";
				zlog_debug(c, "asking system configuration info");
				...
				stampaVideo(&duinoAnswersHead, &duinoAnswersTail, s);
					...
					getvalues(&headPack, &tailPack, s, 1, sensorLst);				
						 }
					...
					else if(strcmp(argv[arguments],"fill")==0){
						/*codice aggiunto da Gian*/
						powerOn("all", sensorLst, s);
						/*---------------------*/
						printf("Richiesta fill hydro\n");
						zlog_debug(c, "fill sent");
						fill(&headPack, &tailPack, s, sensorLst);
					 }
					 else if(argv[arguments] != NULL ){ //other debug commands are simply sent to Server without post processing
...
inline void fill(duinoData **h, duinoData **t, int s, sensorList *sL){
	char* command = (char*) calloc(32,sizeof(char));
	command = "fill hydro";
	sendto(s,command,strlen(command), 0, (struct sockaddr *) &soac, sizeof(soac));
	stampaVideo(h, t, s); //rx( h, t, s, 1, 1);//server answer no need to save for now
	(*h) = (*t) = NULL;
	int index = 0, retry=3;
	sendto(s,"read",sizeof("read"), 0, (struct sockaddr *) &soac, sizeof(soac));
	//continue receiving until msg RAM EMPTY received
	while( 1 ) {
		...
		if(retry==0) {
			zlog_fatal(c, "UDP Server unreachable");
			exit(1);
		}
			continue;
		}
		if( strstr( (*t)->pack, "OK RAM EMPTY") != NULL ){
			printf("received: %s \n", (*t)->pack);
			(*t) = (*t)->prec;
		...
		sendto(s,"read",sizeof("read"), 0, (struct sockaddr *) &soac, sizeof(soac));
		index++;
		printf("Ricevuti %d pacchetti",index);
		printf("\n\033[F\33[J");
	}
		powerOff(NULL, s);
		salvaAudio(h, sL);				
...
void salvaAudio(duinoData **h, sensorList *sL){
	...
	strftime(ufdate, sizeof(ufdate), "%Y%m%d %H:%M:%S%z", data);
	...
	int byteScrit=0;
	int idx=3014;
	while(strcmp(tempData->pack, "MSG=OK RAM EMPTY") != 0 ){
	//while(idx>0){
		idx--;
...
void getvalues(duinoData **h, duinoData **t, int s, int numPack, sensorList * sL){
	...
	duinoData *temp = (*h);
	char c[UDP_PACK_DIM], *ch,*uguale,canale[2],misura[10];
	memset(c,0,sizeof(c));
	memcpy(c,temp->pack,sizeof(temp->pack));
	char *token = strtok(c, ",");
	while(token != NULL){				
		ch = strchr(token, 'h');
		if (ch == NULL)
...
void powerOff(sensorList *sL, int s){
	...
	if (tempSL == NULL){
		strcpy(buffer, "pwr_off=all");
		printf("sending: %s\n",buffer);
		sendto(s,buffer,32, 0, (struct sockaddr *) &soac, sizeof(soac));
		checkAnswer(buffer,s);
...
//un nuovo file. Il nome con cui le salvo ancora non l'ho deciso
	...
	strftime(ufdate, sizeof(ufdate), "%H:%M:%S %z", data);
		...
		while(listaSensori != NULL){
			if(listaSensori->sensore->status == 1){
				...
				while(temp != NULL){
					...
					if( checkAnswer(buffer,s) > 0 ){
							printf("can't power on seafloor sensors, exiting\n");
							zlog_fatal(c, "powerOn: can't power on seafloor sensors, exiting" );
							exit(4);
					}
					//stampaVideo(&duinoRispostaHead, &duinoRispostaTail,s);
									
					zlog_debug(c, "powerOn: channel inserted into list");
					temp = temp->next;
						}
							}// chiusura if status
...
/**
 ...
 @return	integer 0-> success, >0 -> error timeout exceeded
**/
int rx(duinoData **h, duinoData **t, int s, int numPack, int test){
//attende di ricevere numPack pack da UDP_PACK_DIM Bytes
...
int checkAnswer(char* command,int sock){
	...
	int retry=2;
	if (rx(&temph, &tempt, sock, 1, 0) == 0 ){ printf("Server answer:  %s\n", tempt->pack);
