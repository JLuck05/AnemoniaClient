...
int main(int argc, char *argv[]){
	...
	if(s < 0){
		...
		for(arguments=1; arguments<argc;arguments++ ){
			...
			/*else if(strcmp(argv[arguments],"getsetting")==0){
				//printf("Richiesta getsetting: %s\n", argv[arguments]);
				buffer = argv[arguments];
				zlog_debug(c, "Richiesta getsetting");
				sendto(s,buffer,32, 0, (struct sockaddr *) &soac, sizeof(soac));
				stampaVideo(&duinoAnswersHead, &duinoAnswersTail, s);
			
						 }*/
				//else if(( token = strstr(argv[arguments],"setch=") ) != NULL){
